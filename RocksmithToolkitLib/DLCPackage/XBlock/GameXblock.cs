    ...
    public class GameXblock<T> where T : IEntity
    {
        ...
        public static void Generate2014(DLCPackageData info, Stream outStream) {
            ...
            foreach (var arrangement in info.Arrangements) {
                ...
                entity.Properties.Add(new Property2014() { Name = "AlbumArtSmall", Set = new Set() { Value = String.Format(URN_TEMPLATE, TagValue.Image.GetDescription(), TagValue.DDS.GetDescription(), Path.GetFileNameWithoutExtension(info.AlbumArt64)) } });
                entity.Properties.Add(new Property2014() { Name = "AlbumArtMedium", Set = new Set() { Value = String.Format(URN_TEMPLATE, TagValue.Image.GetDescription(), TagValue.DDS.GetDescription(), Path.GetFileNameWithoutExtension(info.AlbumArt128)) } });
                entity.Properties.Add(new Property2014() { Name = "AlbumArtLarge", Set = new Set() { Value = String.Format(URN_TEMPLATE, TagValue.Image.GetDescription(), TagValue.DDS.GetDescription(), Path.GetFileNameWithoutExtension(info.AlbumArt256)) } });
